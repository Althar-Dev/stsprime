...
export default function ContactPage() {
  ...
  const { data: contactData, loading } = useDoc(db ? doc(db, "settings", "contact") : null);
  ...
  const channels = useMemo(() => [
    {
      ...
      href: `https://wa.me/${contactData?.whatsapp || "6281234567890"}?text=Halo%20Admin%20STS%20PRIME%2C%20saya%20butuh%20bantuan`,
    ...
    },
    {
      ...
      href: `mailto:${contactData?.email || "support@stsprime.com"}`,
    ...
    },
    {
      ...
      href: contactData?.telegram ? `https://t.me/${contactData.telegram}` : "https://t.me/stsprime_official",
    ...
    },
    {
      ...
      desc: contactData?.hours || "24 Jam Non-stop setiap hari.",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name || !email || !message) {
      toast({ variant: "destructive", title: "Gagal Mengirim", description: "Harap isi nama, email, dan pesan Anda." });
      return;
    }
    setIsSubmitting(true);
    await new Promise((resolve) => setTimeout(resolve, 1200));
    setIsSubmitting(false);
    toast({ title: "Pesan Terkirim!", description: "Tim Customer Service kami akan merespons pesan Anda sesegera mungkin." });
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10">
              ...
              <Card className="bento-card p-6 sm:p-8 border-border/50 bg-card/30 backdrop-blur-sm rounded-3xl shadow-xl">
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground font-bold">Isi formulir di bawah ini dan tim support kami akan membalas via email.</p>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Nama Lengkap *</Label>
                      <Input placeholder="Nama Anda" value={name} onChange={(e) => setName(e.target.value)} className="h-11 rounded-xl bg-background/50 border-border/60 text-xs sm:text-sm font-bold" />
                    </div>
                    <div className="space-y-2">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Email Aktif *</Label>
                      <Input type="email" placeholder="nama@email.com" value={email} onChange={(e) => setEmail(e.target.value)} className="h-11 rounded-xl bg-background/50 border-border/60 text-xs sm:text-sm font-bold" />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Kategori Pertanyaan</Label>
                    ...
                    <div className="space-y-2">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">ID Order (Opsional)</Label>
                      <Input placeholder="Contoh: STS-89211" value={orderId} onChange={(e) => setOrderId(e.target.value)} className="h-11 rounded-xl bg-background/50 border-border/60 text-xs sm:text-sm font-bold font-mono" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Pesan atau Detail Kendala *</Label>
                    <Textarea placeholder="Jelaskan detail pertanyaan atau kendala Anda..." rows={4} value={message} onChange={(e) => setMessage(e.target.value)} className="rounded-xl bg-background/50 border-border/60 text-xs sm:text-sm font-bold resize-none" />
                  </div>
                  <Button type="submit" disabled={isSubmitting} className="w-full h-12 rounded-xl font-black text-xs uppercase tracking-widest gap-2 bg-primary text-primary-foreground shadow-lg shadow-primary/20 hover:scale-[1.02] transition-transform">
                    {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Send className="h-4 w-4" /> Kirim Pesan Support</>}
                  </Button>
                </form>
              </Card>
            </div>
            <div className="lg:col-span-5 space-y-6">
              <Card className="bento-card p-6 sm:p-8 border-border/50 bg-card/30 backdrop-blur-sm rounded-3xl space-y-5">
                <div className="space-y-2">
                  <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 text-[9px] font-black uppercase tracking-widest">Garansi Layanan</Badge>
                  <h3 className="font-headline text-lg sm:text-xl font-black text-foreground">Jaminan Respons Cepat</h3>
                  <p className="text-xs text-muted-foreground font-bold leading-relaxed">Setiap laporan kendala transaksi diprioritaskan oleh tim CS teknis kami. Kepuasan dan keamanan Anda adalah prioritas utama STS PRIME.</p>
                </div>
                <div className="space-y-3 pt-2">
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/20 border border-border/40"><CheckCircle2 className="h-5 w-5 text-primary shrink-0" /><span className="text-xs font-bold text-foreground">Verifikasi Otomatis Sistem Kilat</span></div>
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/20 border border-border/40"><ShieldCheck className="h-5 w-5 text-primary shrink-0" /><span className="text-xs font-bold text-foreground">Garansi Uang Kembali Jika Gagal</span></div>
                </div>
              </Card>
              <Card className="bento-card p-6 border-primary/30 bg-gradient-to-br from-primary/10 to-transparent rounded-3xl space-y-3">
                <div className="flex items-center gap-2 text-primary font-black text-xs uppercase tracking-widest"><HelpCircle className="h-4 w-4" /> Pusat Bantuan</div>
                <h4 className="font-headline font-black text-base text-foreground">Cari Jawaban Instan di FAQ</h4>
                <p className="text-xs text-muted-foreground font-bold leading-relaxed">Banyak pertanyaan seputar pengisian item dan pembayaran sudah terbukti dapat diselesaikan langsung di Pusat Bantuan.</p>
                <Link href="/support" className="block pt-1"><Button variant="outline" className="w-full h-10 rounded-xl font-black text-xs border-primary/30 text-primary hover:bg-primary/10">Buka Pusat Bantuan</Button></Link>
              </Card>
            </div>
          </div>
        </section>
